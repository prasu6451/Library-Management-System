...
const updateBook = async (req, res) => {
  try {
    ...
    // Check active issues count for this book
    ...
    if (parsedQuantity < activeIssues) {
      return res.status(400).json({
        message: `Cannot decrease quantity to ${parsedQuantity}. There are currently ${activeIssues} copies of this book issued to students.`
      });
    }

    // New available quantity = total quantity - active issues
...
const deleteBook = async (req, res) => {
  try {
    ...
    if (activeIssues > 0) {
      return res.status(400).json({
        message: 'Cannot delete book. It is currently issued to one or more students.'
      });
    }
